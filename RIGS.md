...
| `lag` | per bone key, how many frames the bone trails its parent (follow-through). Defaults come from the bone's name - ears and hair 2, hair strands and tail 3, hands and gloves 1 - and are here to be tuned. Bones above the head or body in the tree are never lagged, whatever their name. |
| `gaze` | how the pupils move. `source: layer` names the rig's pupil layers, whose bones dart; `source: drawn` gives, per eye, the centre and radius of a disc drawn at render time in `colour`, the darkest colour found in the eye box of the head plate, at 22% of the eye box's height. |
